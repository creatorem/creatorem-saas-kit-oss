...
interface LemonSqueezyWindow extends Window {
    ...
    LemonSqueezy: {
        Setup: (options: { eventHandler: (event: { event: string } | string) => void }) => void;
        Refresh: () => void;
        Url: {
...
export function LemonSqueezyEmbeddedCheckout(props: { checkoutToken: string; onClose?: () => void }) {
    useLoadScript(props.checkoutToken, props.onClose);

    return null;
}

function useLoadScript(checkoutToken: string, onClose: (() => void) | undefined) {
    useEffect(() => {

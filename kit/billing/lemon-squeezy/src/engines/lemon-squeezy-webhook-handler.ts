...
export class LemonSqueezyWebhookHandlerEngine implements AbstractBillingWebhookHandler {
    ...
    constructor(config: BillingConfig, planTypesMap: PriceTypeMap) {
        this.config = config;
    ...
    async handleWebhookEvent(
        ...
        params: {
            ...
            onInvoicePaid: (data: UpsertSubscriptionParams | UpsertOrderParams) => Promise<unknown>;
        ...
        switch (eventName) {
            case 'order_created': {
                return await this.handleOrderCompleted(event as OrderWebhook, params.onCheckoutSessionCompleted);
            }

            case 'subscription_created': {
                return await this.handleSubscriptionCreatedEvent(
            ...
            case 'subscription_updated': {
                return await this.handleSubscriptionUpdatedEvent(
                    event as SubscriptionWebhook,
                    params.onSubscriptionUpdated,
                );
            }
            ...
            case 'subscription_expired': {
                return await this.handleSubscriptionDeletedEvent(
                    event as SubscriptionWebhook,
            ...
            case 'subscription_payment_success': {
                return await this.handleInvoicePaid(event as SubscriptionInvoiceWebhook, params.onInvoicePaid);
            }
    ...
    private async handleSubscriptionCreatedEvent(
        ...
        onSubscriptionCreatedEvent: (data: UpsertSubscriptionParams) => Promise<unknown>,
    ) {
        await initializeLemonSqueezyClient();
        ...
        const payload = payloadBuilderEngine.build({
            customerId,
            id: subscriptionId,
    ...
    private handleSubscriptionUpdatedEvent(
        ...
        return this.handleSubscriptionCreatedEvent(event, onSubscriptionUpdatedCallback);
    }
    ...
    private async handleInvoicePaid(
        ...
        onInvoicePaidCallback: (subscription: UpsertSubscriptionParams) => Promise<unknown>,
    ) {
        await initializeLemonSqueezyClient();
        ...
        const { data: subscriptionResponse } = await getSubscription(subscriptionId);
...
async function isSigningSecretValid(rawBody: string, signatureHeader: string) {
    const { hex: digest } = await createHmac({
        key: envs().LEMON_SQUEEZY_SIGNING_SECRET!,
        data: rawBody,
    });

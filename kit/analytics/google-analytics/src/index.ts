import { envs } from '@kit/google-analytics/envs';
 ...
 */
const GA_MEASUREMENT_ID = envs().NEXT_PUBLIC_GA_MEASUREMENT_ID;

/**
 * Disable tracking on localhost.
 */
const DISABLE_LOCALHOST_TRACKING = envs().NEXT_PUBLIC_GA_DISABLE_LOCALHOST_TRACKING ?? 'true';

/**
 * Disable tracking of page views.
 */
const DISABLE_PAGEVIEWS_TRACKING = envs().NEXT_PUBLIC_GA_DISABLE_PAGE_VIEWS_TRACKING ?? 'false';

/**
 * Create a Google Analytics service.
 */
export function createGoogleAnalyticsEngine(
    props: { measurementId: string; disableLocalhostTracking?: boolean; disablePageViewsTracking?: boolean } = {
    ...
    const disableLocalhostTracking = props.disableLocalhostTracking ?? DISABLE_LOCALHOST_TRACKING === 'true';

    const disablePageViewsTracking = props.disablePageViewsTracking ?? DISABLE_PAGEVIEWS_TRACKING === 'true';
    ...
    return new GoogleAnalyticsEngine(measurementId, disableLocalhostTracking, disablePageViewsTracking);
}

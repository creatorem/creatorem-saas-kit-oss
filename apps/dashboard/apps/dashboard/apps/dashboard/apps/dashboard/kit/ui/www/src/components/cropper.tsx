...
function centerAspectCrop(mediaWidth: number, mediaHeight: number, aspect: number | undefined): PercentCrop {
    return centerCrop(
        aspect
            ? makeAspectCrop(
                  {
...
const Cropper = React.forwardRef<CropperElement, CropperProps>(
    ({ file, aspectRatio, circularCrop, maxImageSize }, ref) => {
        ...
        return (
            <div
                className="flex max-h-full max-w-full items-center justify-center overflow-hidden rounded-sm p-4"
                style={{

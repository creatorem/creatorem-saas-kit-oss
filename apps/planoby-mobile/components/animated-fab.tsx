...
const StyledTouchableWithoutFeedback = withUniwind(TouchableWithoutFeedback);
...
const AnimatedFab: React.FC<AnimatedFabProps> = ({
    ...
    const containerStyle = useAnimatedStyle(() => {
        ...
        const width = interpolate(animation.value, [0, 1], [baseSize, windowWidth - 30], Extrapolate.CLAMP);

        // Use the measured content height for the expanded state
        const height = interpolate(animation.value, [0, 1], [baseSize, contentHeight], Extrapolate.CLAMP);

        // Border radius changes from circle to rounded rectangle
        const borderRadius = interpolate(animation.value, [0, 1], [baseSize / 2, 12], Extrapolate.CLAMP);
    ...
    const bgColor = backgroundColor || colors['--color-primary'];
    ...
    return (
        <>
            ...
            <TouchableWithoutFeedback>
                <StyledAnimatedView
                    className={cn(
                        'absolute z-10 items-center justify-center bg-black shadow-md shadow-black/30 dark:bg-white',
                        getPositionClasses(),
                        className,
                    )}
                    style={[
                ...
                >
                    <TouchableWithoutFeedback onPress={toggleOpen}>
                        <StyledAnimatedView
                            className={cn('absolute z-20 items-center justify-center', iconClassName)}
                    ...
                    <StyledAnimatedView
                        className={cn(
                            'absolute z-10 h-full w-full items-start justify-start p-6 pb-0',
                            contentClassName,
                        )}

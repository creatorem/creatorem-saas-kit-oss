...
const Slider = ({
    ...
    const panGesture = Gesture.Pan()
        ...
        .onUpdate((event) => {
            ...
            let newPercentage = startPercentage.value + event.translationX / usableWidth;
    ...
    const tapGesture = Gesture.Tap().onEnd((event) => {
        runOnJS(calculateValueFromTap)(event.x);
    });
        ...
        <View className={cn('w-full', className)}>
            {label && (
                <View className="mb-2 flex-row justify-between">
                    <Text className={currentSize.labelText}>{label}</Text>
                    <Animated.Text className={cn('text-foreground', currentSize.valueText)}>
                        {formatValue.value}
                    </Animated.Text>
                </View>
            )}
